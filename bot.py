...
Imkoniyatlar:
  ...
  - Asosiy menyu — Telegram chatning doimiy (pastki) tugmalar paneli sifatida;
    Ha/Yo'q va boshqa kontekstli tanlovlar esa xabarga bog'langan inline
    tugmalar sifatida qoladi
  - Optimallashtirilgan o'z-o'zini tekshirish — progress ko'rsatish shart
    bo'lmagan hollarda bitta chaqiruvda ishlaydi (bir necha barobar tezroq)
...
CUSTOM_LIMITS_FILE = "custom_limits.json"
...
user_pending_result: Dict[int, bytes] = {}     # fayl nomi tanlanguncha tayyor PNG baytlarini saqlash
...
CB_FILENAME_AUTO = "filename_auto"
CB_FILENAME_CUSTOM = "filename_custom"
...
TEXTS = {
    "uz": {
        ...
        "ask_filename_choice": "📁 Fayl uchun nom qanday bo'lsin?",
        "btn_filename_auto": "🤖 Bot o'zi qo'ysin",
        "btn_filename_custom": "✏️ O'zim kirtaman",
        "ask_filename_text": "✏️ Fayl nomini yozing (kengaytmasiz, masalan: mening_sirim):",
        "filename_too_long": "⚠️ Nom juda uzun, qisqartirib qo'ydim.",
        "weak_password": "⚠️ Parol juda qisqa (kamida {min_len} belgi bo'lsin). Qaytadan kiriting:",
        "ask_password_confirm": "🔑 Parolni tasdiqlash uchun qaytadan kiriting:",
        "password_mismatch": "❌ Parollar mos kelmadi. Parolni boshidan kiriting:",
        "you_are_banned": "⛔ Siz botdan foydalanishdan mahrum qilingansiz.",
        "broadcast_usage": "Foydalanish: /broadcast xabar matni",
        "broadcast_done": "📢 Xabar yuborildi: {ok} ta muvaffaqiyatli, {fail} ta xato.",
        "ban_usage": "Foydalanish: /ban user_id",
        "unban_usage": "Foydalanish: /unban user_id",
        "ban_done": "⛔ Foydalanuvchi {uid} bloklandi.",
        "unban_done": "✅ Foydalanuvchi {uid} blokdan chiqarildi.",
        "setlimit_usage": "Foydalanish: /setlimit user_id son (0 = standart limitga qaytarish)",
        "setlimit_done": "✅ Foydalanuvchi {uid} uchun kunlik limit: {limit}",
        "invalid_number": "❌ Son noto'g'ri kiritildi.",
    },
    "ru": {
        ...
        "ask_filename_choice": "📁 Какое имя дать файлу?",
        "btn_filename_auto": "🤖 Пусть бот выберет сам",
        "btn_filename_custom": "✏️ Я введу сам",
        "ask_filename_text": "✏️ Введите имя файла (без расширения, например: мой_секрет):",
        "filename_too_long": "⚠️ Имя слишком длинное, я его сократил.",
        "weak_password": "⚠️ Пароль слишком короткий (минимум {min_len} символов). Введите снова:",
        "ask_password_confirm": "🔑 Введите пароль ещё раз для подтверждения:",
        "password_mismatch": "❌ Пароли не совпадают. Введите пароль заново:",
        "you_are_banned": "⛔ Вам запрещено пользоваться этим ботом.",
        ...
        "setlimit_usage": "Использование: /setlimit user_id число (0 = сбросить на стандартный лимит)",
        "setlimit_done": "✅ Дневной лимит для {uid}: {limit}",
        "invalid_number": "❌ Введено некорректное число.",
    },
    "en": {
        ...
        "ask_filename_choice": "📁 What should the file be named?",
        "btn_filename_auto": "🤖 Let the bot choose",
        "btn_filename_custom": "✏️ I'll type it myself",
        "ask_filename_text": "✏️ Type the file name (no extension, e.g.: my_secret):",
        "filename_too_long": "⚠️ The name was too long, I shortened it.",
        "weak_password": "⚠️ Password is too short (minimum {min_len} characters). Enter it again:",
        "ask_password_confirm": "🔑 Enter the password again to confirm:",
        "password_mismatch": "❌ Passwords didn't match. Enter the password again:",
        "you_are_banned": "⛔ You are banned from using this bot.",
        "broadcast_usage": "Usage: /broadcast message text",
        "broadcast_done": "📢 Broadcast finished: {ok} succeeded, {fail} failed.",
        "ban_usage": "Usage: /ban user_id",
        "unban_usage": "Usage: /unban user_id",
        "ban_done": "⛔ User {uid} has been banned.",
        "unban_done": "✅ User {uid} has been unbanned.",
        "setlimit_usage": "Usage: /setlimit user_id number (0 = reset to default limit)",
        "setlimit_done": "✅ Daily limit for {uid}: {limit}",
...
# ============ SHAXSIY LIMITLAR (admin /setlimit orqali sozlaydi) ============

def load_custom_limits() -> dict:
    try:
...
def save_custom_limits():
    try:
        with open(CUSTOM_LIMITS_FILE, "w", encoding="utf-8") as f:
            json.dump(custom_limits, f)
    except Exception as e:
        logger.warning("Shaxsiy limitlarni saqlab bo'lmadi: %s", e)


custom_limits: dict = load_custom_limits()


def get_daily_limit_for(user_id: int) -> int:
    return custom_limits.get(str(user_id), DAILY_LIMIT)
...
# ============ FOYDALANUVCHILARNI BLOKLASH (BAN) ============
...
def save_banned():
    try:
        ...
        logger.warning("Ban ro'yxatini saqlab bo'lmadi: %s", e)
...
async def extract_payload_from_image_progress(image: Image.Image, on_progress=None) -> Optional[str]:
    ...
    if on_progress is None:
        # TEZKOR YO'L: progress ko'rsatish shart bo'lmasa (masalan, ichki
        # o'z-o'zini tekshiruvda), 5 bosqichli executor aylanishlariga hojat
        # yo'q — bitta chaqiruvda butun massivni skanerlaymiz. Bu har bir
        # "yashirish" amalidan keyingi avtomatik tekshiruvni ~5x tezlashtiradi.
        text_all = await loop.run_in_executor(EXECUTOR, _extract_chunk_sync, flat, len(flat))
        if END_MARKER in text_all:
            return text_all.split(END_MARKER)[0]
        return None
...
def main_reply_keyboard(user_id: int) -> ReplyKeyboardMarkup:
    """
    Asosiy menyu — Telegram chatning pastki doimiy tugmalar paneli sifatida
    (inline emas). Bir marta yuborilgach, foydalanuvchi uni istalgan vaqtda
    ko'rib turadi va bosishi mumkin; qayta yuborishning hojati yo'q.
    """
    keyboard = [
        [KeyboardButton(t(user_id, "btn_hide")), KeyboardButton(t(user_id, "btn_extract"))],
        [KeyboardButton(t(user_id, "btn_help")), KeyboardButton(t(user_id, "btn_about"))],
        [KeyboardButton(t(user_id, "btn_lang"))],
    ]
...
def match_menu_action(text: str) -> Optional[str]:
    """
    Doimiy menyudagi tugma matnini (istalgan tildagi) amal turiga mos keladi.
    ReplyKeyboard tugmalari oddiy matn xabari sifatida keladi, callback_data
    orqali emas — shuning uchun matn bo'yicha solishtiramiz.
    """
    for lang_texts in TEXTS.values():
        for key, action in _MENU_ACTION_KEYS.items():
            if text == lang_texts.get(key):
                return action
    return None
...
def filename_choice_keyboard(user_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [[
            InlineKeyboardButton(t(user_id, "btn_filename_auto"), callback_data=CB_FILENAME_AUTO),
            InlineKeyboardButton(t(user_id, "btn_filename_custom"), callback_data=CB_FILENAME_CUSTOM),
        ]]
    )


def default_filename() -> str:
    return f"stego_{datetime.now():%Y%m%d_%H%M%S}.png"


def sanitize_filename(raw_name: str) -> str:
    """Foydalanuvchi kiritgan nomni xavfsiz fayl nomiga aylantiradi."""
    name = raw_name.strip()
    # Fayl tizimlarida taqiqlangan belgilarni olib tashlash
    for ch in '\\/:*?"<>|\n\r\t':
...
def reset_user_state(user_id: int):
    ...
    user_pending_result.pop(user_id, None)


async def send_final_result(user_id: int, chat_id: int, context: ContextTypes.DEFAULT_TYPE, filename: str):
    """Tayyor PNG'ni tanlangan fayl nomi bilan yuboradi va holatni tozalaydi."""
    data = user_pending_result.get(user_id)
    if data is None:
        await context.bot.send_message(chat_id=chat_id, text=t(user_id, "no_image_found"), reply_markup=main_reply_keyboard(user_id))
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        t(user_id, "welcome"), reply_markup=main_reply_keyboard(user_id), parse_mode="HTML"
    )
...
async def cancel_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(user_id, key), reply_markup=main_reply_keyboard(user_id))
...
async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin barcha foydalanuvchilarga bir vaqtda xabar yuborishi uchun."""
    user_id = update.effective_user.id
    if not is_admin(user_id):
        await update.message.reply_text(t(user_id, "stats_admin_only"))
        return
    ...
    for uid in list(stats["users"]):
        try:
            await api_call(lambda uid=uid: context.bot.send_message(chat_id=uid, text=text))
            ok += 1
        except Exception:
            fail += 1
        await asyncio.sleep(0.05)  # Telegram flood-limitiga tushib qolmaslik uchun

    await update.message.reply_text(t(user_id, "broadcast_done", ok=ok, fail=fail))
...
async def ban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    ban_user(target_id)
    await update.message.reply_text(t(user_id, "ban_done", uid=target_id))
...
async def unban_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(t(user_id, "unban_done", uid=target_id))


@flood_guard
async def setlimit_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin muayyan foydalanuvchi uchun kunlik limitni belgilaydi (0 = standartga qaytarish)."""
    user_id = update.effective_user.id
    if not is_admin(user_id):
        await update.message.reply_text(t(user_id, "stats_admin_only"))
        return
    if len(context.args) < 2:
        await update.message.reply_text(t(user_id, "setlimit_usage"))
        return
    try:
        target_id = int(context.args[0])
    ...
    if new_limit <= 0:
        custom_limits.pop(str(target_id), None)
        save_custom_limits()
        await update.message.reply_text(t(user_id, "setlimit_done", uid=target_id, limit=DAILY_LIMIT))
    else:
        custom_limits[str(target_id)] = new_limit
        save_custom_limits()
        await update.message.reply_text(t(user_id, "setlimit_done", uid=target_id, limit=new_limit))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == CB_HIDE_AGAIN:
        if get_remaining_quota(user_id) <= 0:
            await safe_edit(query, t(user_id, "limit_reached", limit=get_daily_limit_for(user_id)),
                             reply_markup=InlineKeyboardMarkup([]))
            return
        reset_user_state(user_id)
        user_states[user_id] = "waiting_for_image_to_hide"
        await query.message.reply_text(
            t(user_id, "prompt_send_image_hide"), reply_markup=cancel_keyboard(user_id), parse_mode="HTML"
        )
        ...
        await safe_edit(query, t(user_id, "lang_set"), reply_markup=InlineKeyboardMarkup([]))
        # Yangi til bilan menyu tugmalarini yangilab, doimiy menyuni qayta ko'rsatamiz
        await query.message.reply_text(
            t(user_id, "welcome"), reply_markup=main_reply_keyboard(user_id), parse_mode="HTML"
        )
    ...
    elif data == CB_FILENAME_AUTO:
        await send_final_result(user_id, query.message.chat_id, context, default_filename())

    elif data == CB_FILENAME_CUSTOM:
        user_states[user_id] = "waiting_for_filename_text"
        await safe_edit(query, t(user_id, "ask_filename_text"), reply_markup=cancel_keyboard(user_id))

    elif data in (CB_CANCEL, CB_BACK):
        reset_user_state(user_id)
        # Doimiy (reply) menyu allaqachon ko'rinib turibdi — faqat eski inline
        # tugmalarni (masalan, Bekor qilish/Orqaga) tozalab, matnni yangilaymiz
        await safe_edit(query, t(user_id, "welcome"), reply_markup=InlineKeyboardMarkup([]), parse_mode="HTML")
...
async def handle_photo_or_document(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state not in ("waiting_for_image_to_hide", "waiting_for_image_to_extract"):
        await update.message.reply_text(t(user_id, "select_from_menu_first"), reply_markup=main_reply_keyboard(user_id))
        return
    ...
    if state == "waiting_for_image_to_hide":
        ...
        if not payload:
            ...
            await update.message.reply_text(t(user_id, "choose_from_menu"), reply_markup=main_reply_keyboard(user_id))
            return
...
async def handle_menu_action(action: str, update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    """Doimiy (reply) menyudagi tugma bosilganda tegishli amalni bajaradi."""
    if action == "hide":
        if get_remaining_quota(user_id) <= 0:
            await update.message.reply_text(t(user_id, "limit_reached", limit=get_daily_limit_for(user_id)))
            return
        reset_user_state(user_id)
        user_states[user_id] = "waiting_for_image_to_hide"
        await update.message.reply_text(
        ...
        await update.message.reply_text(t(user_id, "prompt_send_image_extract"), reply_markup=cancel_keyboard(user_id))
    elif action == "help":
        await update.message.reply_text(t(user_id, "help_text"), reply_markup=back_keyboard(user_id), parse_mode="HTML")
    elif action == "about":
        await update.message.reply_text(t(user_id, "about_text"), reply_markup=back_keyboard(user_id), parse_mode="HTML")
    elif action == "lang":
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Doimiy menyu tugmasi bosildimi? (har qanday holatni to'xtatib, ustunlik qiladi) ---
    menu_action = match_menu_action(message_text)
    if menu_action:
        await handle_menu_action(menu_action, update, context, user_id)
        return

    # --- Fayl nomini qo'lda kiritish ---
    if state == "waiting_for_filename_text":
        filename = sanitize_filename(message_text)
        await send_final_result(user_id, update.effective_chat.id, context, filename)
        return
    ...
    if state == "waiting_for_feedback":
        ...
        await update.message.reply_text(t(user_id, "feedback_thanks"), reply_markup=main_reply_keyboard(user_id))
        return
    ...
    if state == "waiting_for_password_set":
        ...
        if len(password) < MIN_PASSWORD_LEN:
            await update.message.reply_text(
                t(user_id, "weak_password", min_len=MIN_PASSWORD_LEN), reply_markup=cancel_keyboard(user_id)
            )
            return  # shu holatda qoladi, qaytadan kiritishga ruxsat beriladi
        user_secret_text[user_id] = json.dumps({"password": password})
        user_states[user_id] = "waiting_for_password_confirm"
        await update.message.reply_text(t(user_id, "ask_password_confirm"), reply_markup=cancel_keyboard(user_id))
        return

    # --- Parolni tasdiqlash (yashirishda, xato yozib qo'yishning oldini olish uchun) ---
    if state == "waiting_for_password_confirm":
        pending = user_secret_text.get(user_id)
        original_password = json.loads(pending)["password"] if pending else None
        if message_text != original_password:
            user_secret_text.pop(user_id, None)
            user_states[user_id] = "waiting_for_password_set"
            await update.message.reply_text(t(user_id, "password_mismatch"))
            return
    ...
    if state in ("waiting_for_text", "waiting_for_text_with_password"):
        ...
        if image is None:
            reset_user_state(user_id)
            await update.message.reply_text(t(user_id, "no_image_found"), reply_markup=main_reply_keyboard(user_id))
            return
        ...
        user_pending_result[user_id] = buffer.getvalue()
        ...
        user_states[user_id] = "waiting_for_filename_choice"
        await update.message.reply_text(
            t(user_id, "ask_filename_choice"), reply_markup=filename_choice_keyboard(user_id)
        )
        return
